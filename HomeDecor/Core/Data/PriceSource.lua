...
function PriceSource.FlushPriceCache()
  ...
  wipe(atrPriceCache)
  cacheTimestamp = time()
  PriceSource.revision = PriceSource.revision + 1
end
...
function PriceSource.SetPreferredSource(name)
  local g = db()
  if g then
    local nextSource = (name == "TSM" or name == "Auctionator") and name or nil
    if g.preferredSource ~= nextSource then
      g.preferredSource = nextSource
