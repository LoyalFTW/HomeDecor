...
function Tracker:NotifyChanged()
  self.revision = self.revision + 1
  if NS.Systems.MapPins then
...
function Tracker:ClearIfUnlisted(record)
  local profile = NS.Systems.Database:GetProfile()
  local key = self:Key(record)
  if not profile or not key or profile.tracked[key] ~= true then return false end
  if NS.Systems.Lists:ContainsAny(record) then return false end
  profile.tracked[key] = nil
  self:NotifyChanged()
  return true
end
