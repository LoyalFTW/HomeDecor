...
local function migrateRoom(room, index)
  if type(room) ~= "table" then return nil end
  local key = room.templateKey or room.template
  local template = key and TEMPLATE_BY_KEY[key]
  if not template and key then
    local alias = TEMPLATE_ALIASES[tostring(key):lower()]
    template = alias and TEMPLATE_BY_KEY[alias]
  end
  if template then
    room.templateKey = template.key
    if room.w == nil then room.w = template.w end
    if room.h == nil then room.h = template.h end
    if room.color == nil or type(room.color) == "table" then room.color = template.color end
    if room.shape == nil then room.shape = template.shape end
    if room.cost == nil then room.cost = template.cost end
    if room.connections == nil then room.connections = template.connections end
    if room.art == nil then room.art = template.art end
    if room.icon == nil then room.icon = template.icon end
    if trim(room.name) == "" then room.name = template.name end
  end
  room.id = tonumber(room.id) or index
  room.name = trim(room.name) ~= "" and room.name or ("Room " .. tostring(index))
  room.design = room.design or ""
  room.x = tonumber(room.x) or 0
  room.y = tonumber(room.y) or 0
  room.floor = tonumber(room.floor) or tonumber(room.capture and room.capture.floor) or 1
  room.rotation = tonumber(room.rotation) or 0
  room.budgetLimit = tonumber(room.budgetLimit) or 30
  room.items = type(room.items) == "table" and room.items or {}
  return room
end

local function migrateArchitectStore(db)
  db.layouts = type(db.layouts) == "table" and db.layouts or {}
  local maxLayoutID, maxRoomID = 0, 0
  for layoutIndex, layout in ipairs(db.layouts) do
    if type(layout) == "table" then
      layout.id = tonumber(layout.id) or layoutIndex
      maxLayoutID = math.max(maxLayoutID, layout.id)
      layout.name = trim(layout.name) ~= "" and layout.name or ("Layout " .. tostring(layout.id))
      layout.gridW = tonumber(layout.gridW) or 24
      layout.gridH = tonumber(layout.gridH) or 16
      layout.budgetLimit = tonumber(layout.budgetLimit) or 250
      layout.rooms = type(layout.rooms) == "table" and layout.rooms or {}
      for roomIndex, room in ipairs(layout.rooms) do
        room = migrateRoom(room, roomIndex)
        layout.rooms[roomIndex] = room
        if room then maxRoomID = math.max(maxRoomID, room.id) end
      end
    end
  end
  db.nextLayoutID = math.max(tonumber(db.nextLayoutID) or 1, maxLayoutID + 1)
  db.nextRoomID = math.max(tonumber(db.nextRoomID) or 1, maxRoomID + 1)
  db.architectSchemaVersion = 1
end
...
local function itemIcon(it, decorID, itemID)
  if type(it) == "table" and it.icon then return it.icon end
  local VD = NS.UI and NS.UI.Viewer and NS.UI.Viewer.Data
...
function Architect:AddItemToRoom(room, info, qty)
  if type(room) ~= "table" then
    room = findRoom(self:GetActiveLayout(), room)
  end
  ...
  local item = {
    ...
    name = info.name or info.title or itemName(info.raw or info, decorID),
    icon = info.icon,
...
function Architect:RoomBudget(room)
  ...
  for _, item in ipairs((room and room.items) or {}) do
    ...
    if Catalog then
      for _, candidate in ipairs(Catalog.ordered or {}) do
        if tonumber(candidate.decorID) == tonumber(item.decorID) then
          record = candidate
          break
        end
      end
    end
    local isOwned = Collection and ((Collection.IsDecorCollected and Collection:IsDecorCollected(item.decorID)) or (record and Collection.IsOwned and Collection:IsOwned(record)))
...
function Architect:ImportAny(text, name)
  ...
  local Catalog = NS.Systems and NS.Systems.Catalog
  if Catalog then
    for _, entry in ipairs(Catalog.ordered or {}) do
      visitNode(entry, out, seen)
    end
  end
...
local function currentHouseKey(info)
  if type(info) ~= "table" then return nil end
  if info.houseGUID ~= nil then return "house:" .. tostring(info.houseGUID) end
  if info.neighborhoodGUID ~= nil and info.plotID ~= nil then
    return "plot:" .. tostring(info.neighborhoodGUID) .. ":" .. tostring(info.plotID)
  end
  if info.ownerName ~= nil and (info.houseName ~= nil or info.name ~= nil) then
    return "owner:" .. tostring(info.ownerName) .. ":" .. tostring(info.houseName or info.name)
  end
end
...
local function housingFloorCount()
  if not C_HousingLayout then return nil end
  if C_HousingLayout.GetNumFloors then return C_HousingLayout.GetNumFloors() end
  if C_HousingLayout.GetHighestOccupiedFloorIndex and C_HousingLayout.GetLowestOccupiedFloorIndex then
    local highest = C_HousingLayout.GetHighestOccupiedFloorIndex()
    local lowest = C_HousingLayout.GetLowestOccupiedFloorIndex()
    if highest ~= nil and lowest ~= nil then return highest - lowest + 1 end
  end
  return nil
end
...
local function capturedRoomGUID(room)
  local guid = room and room.capture and room.capture.roomGUID
  if guid == nil or tostring(guid) == "" then return nil end
  return tostring(guid)
end

local function findCapturedLayout(db, captured, houseKey)
  local incoming = {}
  for _, room in ipairs(captured or {}) do
    if room.roomGUID ~= nil then incoming[tostring(room.roomGUID)] = true end
  end
  if houseKey then
    for _, layout in ipairs((db and db.layouts) or {}) do
      if layout.captureHouseKey == houseKey then return layout end
    end
  end
  local best, bestMatches
  for _, layout in ipairs((db and db.layouts) or {}) do
    local matches = 0
    if not houseKey or layout.captureHouseKey == nil or layout.captureHouseKey == houseKey then
      for _, room in ipairs(layout.rooms or {}) do
        local guid = capturedRoomGUID(room)
        if guid and incoming[guid] then matches = matches + 1 end
      end
    end
    if matches > 0 and (not bestMatches or matches > bestMatches) then
      best, bestMatches = layout, matches
    end
  end
  return best
end

local function preserveCapturedRoom(oldRoom, room)
  if not oldRoom then return end
  room.id = oldRoom.id or room.id
  room.name = oldRoom.name or room.name
  room.design = oldRoom.design or room.design
  room.items = type(oldRoom.items) == "table" and oldRoom.items or room.items
  room.budgetLimit = oldRoom.budgetLimit or room.budgetLimit
  room.worldBounds = oldRoom.worldBounds
  room.externalID = oldRoom.externalID
  room.exportID = oldRoom.exportID
  room.isPrimaryStairwell = oldRoom.isPrimaryStairwell
  room.primaryStairwellId = oldRoom.primaryStairwellId
  room.interFloorConnection = oldRoom.interFloorConnection
end
...
function Architect:BuildLayoutFromPinCaptures(name, captures, house)
  local captured = {}
  ...
  local db = ensureProfile()
  local houseKey = currentHouseKey(house)
  local layout = findCapturedLayout(db, captured, houseKey)
  local oldByGUID = {}
  if layout then
    for _, room in ipairs(layout.rooms or {}) do
      local guid = capturedRoomGUID(room)
      if guid then oldByGUID[guid] = room end
    end
    layout.rooms = {}
    db.activeLayoutID = layout.id
  else
    layout = self:CreateLayout(name or "Captured House")
  end
  layout.captureHouseKey = houseKey or layout.captureHouseKey
  layout.rooms = {}
  ...
  for _, raw in ipairs(captured) do
    ...
    preserveCapturedRoom(oldByGUID[tostring(raw.roomGUID)], room)
...
function Architect:_FinishLiveCapture(captures, reason)
  ...
  local layout = self:BuildLayoutFromPinCaptures(name, captures, house)
  local roomCount = 0
  local floorCounts = {}
...
function Architect:StartLiveCapture(reason)
  ...
  local maxFloor = housingFloorCount()
  if not maxFloor then return nil, "Floor count is unavailable. Re-enter the house and try again." end
...
function Architect:FindLayoutByID(id)
  return findLayout(ensureProfile(), id)
end

function Architect:FindLayoutByName(name)
  local db = ensureProfile()
  for _, layout in ipairs((db and db.layouts) or {}) do
...
function Architect:GetRoom(id)
  return findRoom(self:GetActiveLayout(), id)
end

function Architect:GetTemplate(value)
  return self:ResolveTemplate(value)
end
