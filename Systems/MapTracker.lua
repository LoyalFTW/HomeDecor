...
local function buildIndex()
  ...
  local function walk(node)
    ...
    if isVendorRecord(node) then
      ...
      else out[#out+1] = node end
      return
    end
    for _, v in pairs(node) do if type(v) == "table" then walk(v) end end
  end
  walk(root)
  if #out == 0 then return end
  indexBuilt = true
  wipe(vendorsByMapID); wipe(vendorsByZoneName); wipe(vendorsByZoneKey)
  for i = 1, #out do
    local v = out[i]
...
local function getContinentChild(continentID, zoneName)
  ...
  if not byContinent then byContinent = {}; continentChildren[continentID] = byContinent end
  ...
  if C_Map and C_Map.GetMapChildrenInfo then
    ...
    if type(children) == "table" then
      for i = 1, #children do
        local c = children[i]
        if c and c.mapID and c.name == zoneName then child = c.mapID; break end
      end
    end
  end
...
local function fire(zoneName, mapID)
  for _, fn in pairs(callbacks) do pcall(fn, zoneName, mapID) end
end
...
function MapTracker:QueueUpdate(delay, force)
  ...
  if self.queued then
    ...
    if delay > (self._rerunDelay or 0) then self._rerunDelay = delay end
    return
  end
  self.queued = true
  ...
  C_Timer.After(delay, function()
    if not MapTracker.enabled then
      MapTracker.queued = false; MapTracker._queuedForce = nil
      MapTracker._rerun = nil; MapTracker._rerunForce = nil; MapTracker._rerunDelay = nil
      return
    end
    MapTracker.queued = false
    local runForce = MapTracker._queuedForce; MapTracker._queuedForce = nil
...
function MapTracker:Enable(enabled)
  ...
  if not enabled then
    NS.UnregisterEvent(MapTracker, "PLAYER_ENTERING_WORLD")
    NS.UnregisterEvent(MapTracker, "ZONE_CHANGED_NEW_AREA")
    NS.UnregisterEvent(MapTracker, "ZONE_CHANGED")
    NS.UnregisterEvent(MapTracker, "ZONE_CHANGED_INDOORS")
    NS.UnregisterEvent(MapTracker, "ADDON_LOADED")
    return
  end
  ...
  NS.RegisterEvent(MapTracker, "PLAYER_ENTERING_WORLD", function()
    MapTracker:QueueUpdate(0.90, true)
  end)
  NS.RegisterEvent(MapTracker, "ZONE_CHANGED_NEW_AREA", function()
    MapTracker:QueueUpdate(0.65, true)
  end)
  NS.RegisterEvent(MapTracker, "ZONE_CHANGED", function()
    MapTracker:QueueUpdate(0.35, true)
  end)
  NS.RegisterEvent(MapTracker, "ZONE_CHANGED_INDOORS", function()
    MapTracker:QueueUpdate(0.35, true)
  end)
  NS.RegisterEvent(MapTracker, "ADDON_LOADED", function(arg1)
...
function MapTracker:ClearCaches()
  indexBuilt = false
  wipe(vendorsByMapID); wipe(vendorsByZoneName); wipe(vendorsByZoneKey); wipe(continentChildren)
end
...
function MapTracker:GetVendorsForCurrentZone()
  ...
  if name and name ~= "" then
    ...
    if zk then local t3 = vendorsByZoneKey[zk]; if t3 then return t3 end end
  end
  return {}
end
...
function MapTracker:CountVendors(vendors)
  ...
  for i = 1, #vendors do
    local c, t = self:CountVendor(vendors[i]); collected = collected + c; total = total + t
  end
  return collected, total
end
